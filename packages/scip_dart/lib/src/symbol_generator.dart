...
class SymbolGenerator {
  ...
  String? symbolFor(Element element) {
    ...
    // MultiplyDefinedElementImpl doesn't implement isPrivate, so check type first
    try {
      if (element.isPrivate && element is! LibraryImportElement) {
        return _localSymbolFor(element);
      }
    } on UnimplementedError {
      // Some element types (like MultiplyDefinedElementImpl) don't implement isPrivate
      // Treat them as non-private to avoid crashes when indexing SDK code
    }
